import { EltCustomElement, o, register, e, attr, CustomElementAttributes, Renderable, node_append } from "elt"
...
export class EltSlTabgroup extends EltCustomElement {
  ...
  set value(tabname: string) {
    ...
    if (new_children == null) {
      this._o_style.set({width: "0", translate: "0px"})
    } else {
      for (let ch of new_children) {
        if (ch instanceof EltSlTab) {
          ...
          this._o_style.set({
            width: `${bbox.width}px`,
            translate: `${bbox.left - pbox.left}px`,
          })
        }
        ch.active = true
      }
  ...
  protected _o_style = o({ width: "0px", translate: "0px" })
